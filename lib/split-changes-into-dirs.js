...
function filterChangedDirectories(files, parentFolder, depth, includeParent) {
    ...
    for (const file of filteredFiles) {
        ...
        const pathParts = relativeFileDirname.split("/");
        let directory = "";
    ...
    var result = changedDirectories.filter((item, index) => changedDirectories.indexOf(item) === index);
    return result;
}
async function run() {
    var _a, _b, _c, _d;
    try {
        ...
        const includeParent = core.getInput("includeParent", { required: true });
        const depth = core.getInput("depth", { required: true });
        ...
        if (overrideDirectories && overrideDirectories !== "[]") {
            ...
            responseDirectories = filterChangedDirectories(responseFiles, baseFolder, Number(depth), Boolean(includeParent));
        }
